return {
	...
	Copy_setclipboard_copyclipboard_setclip_copyclip_settoclipboard_copytoclipboard_ctc_stc_sc_cc_position_teleportstring_copyname_copyusername_copyid_copyuserid_copyid_jobid_copyname_copyuser_copycreatorid_copycreator_copyposition_copypos = {
		...
		Variables = {
			PlaceInfo = valid.table(
				select(2, pcall(service("Marketplace").GetProductInfo, service("Marketplace"), game.PlaceId)),
				{ Creator = {} }
			),
		},
		...
		Function = function(Variables, Speed)
			...
			if not Variables.Enabled then
				...
				Variables.Connection = connect(service("Run").Heartbeat, function(Delta)
					...
					if
						0 < Humanoid.MoveDirection.Magnitude
						and table.find({
							"Landed",
							"Jumping",
							"Running",
					...
					then
						character:TranslateBy(
							Humanoid.MoveDirection * math.min(Delta, 1 / 15) * (Variables.Speed - Humanoid.WalkSpeed)
						)
					end
				end)
		...
		Function = function()
			...
			while #Servers < 1 do
				local Success, Result = pcall(
					game.HttpGet,
					game,
					`https://games.roblox.com/v1/games/{game.PlaceId}/servers/Public?limit=100{if 0 < #Page then "&cursor=" else ""}{Page}`,
					true
				)
				if not Success then
				...
				for _, ServerInfo in UnfilteredServers do
					ServerCount += 1
					if
						(ServerInfo.playing or 0) < (ServerInfo.maxPlayers or 0)
						and 0 < (ServerInfo.playing or 0)
						and ServerInfo.id ~= game.JobId
					then
	...
	FullBrightness_fullbright_bright_fb_nightvision_nightvis_nightv_nvision_nv_fullvision = {
		...
		Toggles = "NoBrightness_unfullbrightness_nofullbrightness_nofullbright_unfullbright_unfb\z
		_unnightvision_nonightvision_novision_nonightvis_unnightvis_nonightv_unnightv_nonvision\z
		_unnvision_unnv_nonv_nov",
		ToggleCheck = true,
		...
		Function = function(Variables, Enabled)
			if Enabled then
				...
				Variables.Connection = connect(service("Run").Heartbeat, function()
					...
					if character then
						local Humanoid = getHumanoid(character, 0.1)
						if Humanoid and 0 < Humanoid.MoveDirection.Magnitude then
							...
							if HumanoidRootPart then
								HumanoidRootPart.CFrame = deltaLerp(
									Pivot,
									CFrame.lookAt(Pivot.Position, Pivot.Position + Humanoid.MoveDirection),
									math.pi / 20,
									Variables.Delta
								)
							else
