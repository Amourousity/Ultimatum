...
do
	...
	local Settings = DecodeJSON(isfile and isfile"Settings.Ultimatum" and readfile"Settings.Ultimatum":gsub("^%bA{","{"),DefaultSettings)
	...
	local InputBegan,InputEnded,Removed = Connect(Frame.InputBegan,function(Input,Ignore)
		if not Ignore and not Debounce and Input.UserInputType.Name == "MouseButton1" then
			...
			DragConnection = Connect(Service"Run".RenderStepped,function()
				...
				local NewPosition = Vector2.new(math.clamp(MousePosition.X,math.floor(CardSize.X/2),math.ceil(ScreenSize.X-CardSize.X/2)),math.clamp(MousePosition.Y,math.floor(CardSize.Y/2),math.ceil(ScreenSize.Y-CardSize.Y/2)))
				if NewPosition ~= MousePosition then
					if mousemoverel and Focused then
