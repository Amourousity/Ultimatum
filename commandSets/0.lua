	...
	ServerHop_serverh_sh_hopserver_hops_hserver_shop = {
		Function = function()
			...
			notify({
				...
				text = `Took {convertTime(os.clock() - Start)} to search {ServerCount} servers ({ViableServerCount} viable). Server <i>{Server.id}</i> has {Server.playing}/{Server.maxPlayers} players`,
			})
